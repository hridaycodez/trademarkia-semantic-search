#!/usr/bin/env python3
"""
analyze_clusters.py — Cluster quality analysis for 20NG GMM clustering.
"""
...
def main():
    ...
    for c in range(min(n_clusters, 5)):
        mask = dominant == c
        if mask.sum() == 0:
            continue
    ...
    print(f"  {'doc':>6}  {'category':30}  {'top cluster':20}  {'2nd cluster':20}")
    for idx in boundary:
    ...
    print_separator("ENTROPY DISTRIBUTION")
    ...
    print(f"  Docs with entropy > 2.0: {(entropy > 2.0).mean():.1%}")
